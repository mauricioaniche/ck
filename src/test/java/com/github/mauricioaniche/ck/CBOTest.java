...
public class CBOTest extends BaseTest {
    ...
    public void couplingWithGenericsAndJavaType() {
        CKClassResult b = report.get("cbo.Coupling61");
        Assert.assertEquals(1, b.getCbo());

        CKClassResult c = report.get("cbo.Coupling62");
        Assert.assertEquals(1, c.getCbo());

		CKClassResult d = report.get("cbo.Coupling63");
		Assert.assertEquals(2, c.getCbo());
